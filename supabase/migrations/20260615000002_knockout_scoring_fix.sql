-- ── Corrected knockout scoring logic ─────────────────────────────────────────
--
-- KNOCKOUT RULES (simplified):
--
--   User predicted DRAW (any score) + picked penalty winner:
--     Game IS a draw  + correct penalty winner  = 5 pts
--     Game IS a draw  + wrong penalty winner     = 2 pts
--     Game NOT a draw (direct win)               = 0 pts  ← key change
--
--   User predicted DIRECT WIN (no penalty picker shown):
--     Exact score + correct team wins in 90 min = 5 pts
--     Wrong score + correct team wins in 90 min = 2 pts
--     Game goes to penalties + their team wins pens = 0 pts ← direct win predicted, didn't happen
--     Wrong team advances (any way)              = 0 pts
--
...
DECLARE
  ...
  v_actual_90min_winner TEXT;        -- 'home' | 'away' | 'draw'
...
BEGIN
  ...
  -- Determine actual 90-minute result
  ...
  LOOP
    ...
    IF v_is_knockout THEN
      ...
      IF rec.predicted_winner = 'draw' THEN
        -- ── User predicted a DRAW and picked a penalty winner ────────────────
        -- Score is irrelevant — only draw vs no-draw and penalty winner matter

        IF v_actual_90min_winner = 'draw' THEN
          -- Game did end in a draw after 90 mins
          IF rec.predicted_penalty_winner = v_penalty_winner THEN
            v_points := 5;   -- Draw predicted + correct penalty winner
          ELSE
            v_points := 2;   -- Draw predicted + wrong penalty winner
          END IF;
        ELSE
          -- Game did NOT go to a draw — user was wrong about match type
          v_points := 0;
        END IF;

      ELSE
        -- ── User predicted a DIRECT WIN (no penalty picker was shown) ────────
        -- Score and advancing team based purely on 90-minute result

        IF v_actual_90min_winner = 'draw' THEN
          -- Game went to penalties — user predicted direct win so they get 0
          -- regardless of which team won on pens
          v_points := 0;
        ELSE
          -- Game was decided in 90 mins
          IF rec.predicted_winner = v_actual_90min_winner THEN
            ...
            END IF;
          ELSE
            v_points := 0;    -- Wrong team
          END IF;
        END IF;
      END IF;

      -- Apply wildcard multiplier for knockout (only 3 outcomes: 5, 2, 0)
      IF rec.wildcard_used THEN
        v_points := CASE v_points
          WHEN 5 THEN 10
          WHEN 2 THEN  4
          ELSE        -3   -- 0 pts with wildcard = -3
        END;
      END IF;

    ELSE
      -- ── GROUP STAGE scoring (completely unchanged) ─────────────────────────
      IF rec.predicted_winner = v_actual_90min_winner THEN
        v_points := 2;
        IF rec.predicted_home_score = v_home_score
           AND rec.predicted_away_score = v_away_score THEN
      ...
      -- Wildcard multiplier for group stage
      IF rec.wildcard_used THEN
        v_points := CASE
    ...
    UPDATE profiles
       SET total_points      = total_points + v_points,
           correct_winners   = correct_winners + CASE
             WHEN v_is_knockout AND v_points IN (5, 10, 2, 4) THEN 1
